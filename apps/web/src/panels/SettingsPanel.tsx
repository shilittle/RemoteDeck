...
export function SettingsPanel(): React.JSX.Element {
  ...
  const shutdown = async (): Promise<void> => {
    if (!window.confirm("停止本机服务会拒绝新任务，并清理本次启动的资源。继续吗？")) return;
    setShutdownBusy(true);
    setError("");
    try {
      await api.shutdown();
      setDiagnosticsMessage("本机服务正在停止；已打开的页面将失去连接。");
    } catch (reason) {
      setError(errorMessage(reason));
    } finally {
    ...
    <section className="panel settings-panel">
      <header className="panel-heading">
        ...
        <div className="button-row wrap">
          ...
          <button className="danger" disabled={shutdownBusy} onClick={() => { void shutdown(); }}>
            {shutdownBusy ? "停止中…" : "停止服务"}
          </button>
        </div>
      </header>
      {error && (
      ...
      <div className="architecture-note">
        ...
        <p>
          本机服务只监听回环地址，使用默认浏览器、Windows OpenSSH 与 ConPTY；不内嵌
          Electron、Chromium、WebView2 或 Node.js 运行时。
        </p>
      </div>
    </section>
  );
