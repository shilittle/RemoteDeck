...
impl TelemetryRegistry {
    ...
    pub async fn stop(&self, app: &AppHandle, host_id: &str) -> AppResult<TelemetryStatus> {
    ...
    async fn stop_all_monitors(&self, app: &AppHandle) {
...
fn run_collector(context: CollectorContext<'_>) -> AppResult<()> {
    ...
    let command = format!("python3 -u - --interval {}", options.interval_seconds);
    let (program, args) = runtime.remote_command_spec(host, command, true)?;
...
impl TelemetryRegistry {
        ...
        if request.signal == "KILL" {
            ...
            let confirmed = rfd::AsyncMessageDialog::new()
                ...
                .set_description(format!(
                    "Host: {}\nPID: {}\nUser: {}\nCommand: {preview}\n\nSIGKILL cannot be handled or deferred by the process. Continue?",
                    host.alias.replace(['\r', '\n'], " "),
                    request.pid,
                    request.expected_user
                ))
            ...
            if confirmed != rfd::MessageDialogResult::Yes {
                return Err(AppError::State("SIGKILL was cancelled".to_owned()));
            }
        }
    ...
    pub async fn remove_host(&self, app: &AppHandle, host_id: &str) -> AppResult<()> {
        self.stop(app, host_id).await?;
    ...
    pub async fn shutdown(&self, app: &AppHandle) {
        ...
        let _ = tokio::time::timeout(BTOP_SHUTDOWN_TIMEOUT, async {
            tokio::join!(self.stop_all_monitors(app), async {
