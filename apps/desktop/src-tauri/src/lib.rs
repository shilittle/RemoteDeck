...
use crate::{
    error::AppResult,
    model::{
        AppSettings, BootstrapPayload, CommandResult, ConnectionTestResult, HostDraft,
        HostKeyCandidate, HostProfile, SettingsPatch, TerminalSnapshot, TunnelDraft, TunnelProfile,
        TunnelSnapshot,
    },
    session::TerminalRegistry,
    ssh::SshRuntime,
    store::AppRepository,
...
fn save_host(state: State<'_, AppState>, draft: HostDraft) -> AppResult<HostProfile> {
    state.repository.save_host(draft)
}
#[tauri::command]
fn delete_host(app: AppHandle, state: State<'_, AppState>, host_id: String) -> AppResult<()> {
    state.terminals.stop_for_host(&host_id);
    state.tunnels.stop_for_host(&app, &host_id);
    state.repository.delete_host(&host_id)
}
#[tauri::command]
async fn scan_host_keys(app: AppHandle, host_id: String) -> AppResult<Vec<HostKeyCandidate>> {
    let (host, ssh) = {
        let state = app.state::<AppState>();
        (state.repository.host(&host_id)?, state.ssh.clone())
    };
...
async fn run_command(
    ...
    ssh.run_command(&host, command, working_directory).await
}
#[tauri::command]
fn start_terminal(
    app: AppHandle,
...
#[tauri::command]
fn write_terminal(state: State<'_, AppState>, session_id: String, data: String) -> AppResult<()> {
...
fn delete_tunnel(app: AppHandle, state: State<'_, AppState>, tunnel_id: String) -> AppResult<()> {
    let _ = state.tunnels.stop(&app, &tunnel_id);
...
#[tauri::command]
fn stop_tunnel(app: AppHandle, state: State<'_, AppState>, tunnel_id: String) -> AppResult<()> {
    state.tunnels.stop(&app, &tunnel_id)
}
#[tauri::command]
fn update_settings(state: State<'_, AppState>, patch: SettingsPatch) -> AppResult<AppSettings> {
    state.repository.update_settings(patch)
}
