...
| M8 | Onboarding, legacy migration, task center, recovery, shortcuts, tray | Complete | `pnpm verify`, `pnpm test:e2e`; 49 unit, 4 integration, 2 Electron E2E; UI-only first-run host→SSH→Codex route, hashed preview/import with disabled cleanup hook and legacy risk rules, unified task recovery, shortcuts, login launch, native tray close/restore/quit, SSH preserved while hidden, responsive states | `feat(m8): integrate onboarding migration tasks and tray` |
