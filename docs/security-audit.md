...
- SSH: unknown host keys stop before authentication; changed keys hard-fail; jump and target are verified independently. Passwords, keyboard answers and passphrases are never persisted and are cleared after use. Generated Ed25519 pairs are parsed and cross-checked before any file is created; bounded regeneration handles invalid dependency output, and a failed generation removes only files created by that request.
...
- Supply chain: lockfile installation is frozen; pnpm permits build scripts only for the pinned Electron/esbuild/ssh2 toolchain entries listed in `pnpm-workspace.yaml`; packaged Electron is taken from that exact local pnpm dependency, with a bounded official-installer retry that fails closed if the executable remains absent; `pnpm audit --audit-level high` reported no known vulnerabilities at audit time. CI receives no application secrets.
