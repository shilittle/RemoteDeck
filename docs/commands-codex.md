...
Non-PTY jobs have bounded concurrency, per-stream 1 MiB capture, timeouts, ordered events, cancellation, and owned-child cleanup. PTY-required jobs are handed to the terminal registry so passwords and interactive prompts never cross HTTP/JSON arguments.
