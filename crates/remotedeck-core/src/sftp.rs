    ...
    pub async fn cancel(&self, job_id: &str) -> AppResult<TransferJob> {
        let (job, control, changed) = {
            ...
            let changed = if matches!(
                ...
                revise_transfer_job(&mut entry.job);
                true
            } else {
                false
            };
            (entry.job.clone(), entry.control.clone(), changed)
        };
        if changed {
            control.cancel();
            self.emit(job.clone());
...
fn revise_transfer_job(job: &mut TransferJob) {
...
mod tests {
    ...
    #[test]
    fn legacy_transfer_snapshot_defaults_revision() {
        let legacy = serde_json::json!({
            "id": "transfer-1",
            "hostId": "host-1",
            "direction": "download",
            "source": "/tmp/source",
            "destination": "C:\\Users\\tester\\Downloads\\source",
            "conflictPolicy": "rename",
            "recursive": false,
            "state": "failed",
            "attempts": 1,
        ...
        let job: TransferJob = serde_json::from_value(legacy).expect("legacy transfer job");
    ...
    #[tokio::test]
    async fn transfer_revision_advances_for_retry_and_cancel_without_noop_events() {
        ...
        let mut entry = failed_download_entry(host.clone());
        let id = entry.job.id.clone();

        let retried = prepare_transfer_retry(&mut entry, host).expect("retry");
        assert_eq!(retried.revision, 2);
        registry.entries.write().insert(id.clone(), entry);
        let mut events = registry.subscribe();

        let cancelling = registry.cancel(&id).await.expect("cancel");
        assert_eq!(cancelling.state, TransferState::Cancelling);
        assert_eq!(cancelling.revision, 3);
        let repeated = registry.cancel(&id).await.expect("repeat cancel");
        assert_eq!(repeated.revision, 3);
        assert_eq!(registry.get(&id).expect("latest job").revision, 3);

        let emitted = events.try_recv().expect("cancelling event").job;
        assert_eq!(emitted.revision, 3);
        assert!(matches!(
            events.try_recv(),
