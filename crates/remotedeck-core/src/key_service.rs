...
fn list_private_keys_blocking() -> AppResult<Vec<PrivateKeyRecord>> {
    ...
    for entry in fs::read_dir(directory)?.take(MAX_KEY_DIRECTORY_ENTRIES) {
        ...
        let mut command = command(&keygen);
...
fn generate_key_blocking(request: KeyGenerateRequest) -> AppResult<KeyOperationResult> {
    ...
    let keygen = find_keygen()?;
    let mut generation = command(&keygen);
    generation
    ...
    let mut verification = command(&keygen);
    verification
        .args(["-y", "-P", "", "-f"])
        .arg(&workspace.private);
...
fn run_owned_child(
    ...
    // Keep this boundary defensive: every current caller uses
    // `process::command`, and future callers cannot accidentally reintroduce
    // a visible Windows console by passing a manually assembled command.
    hide(command);
...
mod tests {
    ...
    fn owned_child_timeout_kills_and_reaps_before_returning() {
        ...
        let mut command = command(powershell);
        command.args([
            "-NoLogo",
            "-NoProfile",
            "-NonInteractive",
