...
impl TunnelRegistry {
    ...
    pub fn stop(&self, events: EventSink, tunnel_id: &str) -> AppResult<()> {
        let _operation = self.operations.lock();
        self.stop_inner(&events, tunnel_id, false)
    }

    fn stop_inner(&self, app: &EventSink, tunnel_id: &str, retire: bool) -> AppResult<()> {
    ...
    pub fn remove(&self, events: EventSink, tunnel_id: &str) -> AppResult<()> {
        let _operation = self.operations.lock();
        self.stop_inner(&events, tunnel_id, true)?;
        self.entries.write().remove(tunnel_id);
        Ok(())
    }

    pub fn stop_for_host(&self, events: EventSink, host_id: &str) -> AppResult<()> {
        let _operation = self.operations.lock();
        let ids = self
            .entries
            .read()
        ...
        for id in ids {
            if let Err(error) = self.stop_inner(&events, &id, true) {
    ...
    pub fn stop_all(&self, events: EventSink) -> AppResult<()> {
        let _operation = self.operations.lock();
        ...
        let errors = thread::scope(|scope| {
            let handles = ids
                ...
                .map(|id| {
                    ...
                    scope.spawn(move || {
                        self.stop_inner(&events, &id, false)
                            .err()
...
fn mark_starting(app: &EventSink, entry: &TunnelEntry, generation: u64) -> bool {
    let pending = {
        let mut inner = entry.inner.lock();
        if !is_current_locked(&inner, generation) {
            return false;
...
fn emit_current(app: &EventSink, entry: &TunnelEntry, generation: u64) -> bool {
    let pending = {
        let inner = entry.inner.lock();
        if !is_current_locked(&inner, generation) {
            return false;
...
fn emit_pending(app: &EventSink, entry: &TunnelEntry, event: &PendingEvent) {
...
fn emit_raw(app: &EventSink, event: TunnelSnapshot) {
