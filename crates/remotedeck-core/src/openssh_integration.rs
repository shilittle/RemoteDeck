...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "requires the disposable Docker OpenSSH fixture"]
async fn sftp_round_trips_recursive_unicode_content_and_cancellation_retry() -> TestResult {
    let config = FixtureConfig::from_environment()?;
    ...
    // Seed a real remote directory so the recursive upload must exercise its
    // transactional overwrite path rather than merely creating a new target.
    let remote_tree = format!("{remote_directory}/递归-远端-δ");
    service
        .mkdir(
            &host,
            SftpMkdirRequest {
                path: remote_tree.clone(),
            },
        )
        .await?;
    let stale_local = workspace.root.join("远端-过期-文件.txt");
    fs::write(&stale_local, "old remote content\n")?;
    let stale_upload = registry.start_upload(
        host.clone(),
        UploadRequest {
            local_path: path_text(&stale_local)?,
            remote_path: format!("{remote_tree}/过期-文件.txt"),
            conflict_policy: ConflictPolicy::Overwrite,
            recursive: false,
        },
    )?;
    assert_completed(&wait_for_transfer(&registry, &stale_upload.id).await?)?;

    let local_tree = workspace.root.join("本地-递归-源-β");
    let nested_tree = local_tree.join("子目录-γ").join("更深-δ");
    fs::create_dir_all(&nested_tree)?;
    let root_content = "RemoteDeck UTF-8 递归往返 ✅\n根文件 αβγ\n".as_bytes();
    let nested_content = "nested content: 数据 / emoji 🙂\n".as_bytes();
    let deep_content = "deep file: Ω≈ç√∫˜µ≤≥÷\n".as_bytes();
    fs::write(local_tree.join("上传-α-数据.txt"), root_content)?;
    fs::write(
        local_tree.join("子目录-γ").join("报告-β.md"),
        nested_content,
    )?;
    fs::write(nested_tree.join("末端-✅.txt"), deep_content)?;

    let upload = registry.start_upload(
        host.clone(),
        UploadRequest {
            local_path: path_text(&local_tree)?,
            remote_path: remote_tree.clone(),
            conflict_policy: ConflictPolicy::Overwrite,
            recursive: true,
        },
    )?;
    assert_completed(&wait_for_transfer(&registry, &upload.id).await?)?;
    ...
    assert!(
        listing
            .entries
            .iter()
            .any(|entry| entry.name == "上传-α-数据.txt")
    );
    assert!(listing.entries.iter().any(|entry| entry.name == "子目录-γ"));
    assert!(
        !listing
            .entries
            .iter()
            .any(|entry| entry.name == "过期-文件.txt")
    );

    // A pre-existing local directory makes the download take its own
    // backup-and-replace overwrite route.
    let local_download = workspace.root.join("下载-递归-副本-γ");
    fs::create_dir_all(&local_download)?;
    fs::write(local_download.join("过期-本地.txt"), "old local content\n")?;
    ...
    assert_completed(&wait_for_transfer(&registry, &download.id).await?)?;
    assert_eq!(
        fs::read(local_download.join("上传-α-数据.txt"))?,
        root_content
    );
    assert_eq!(
        fs::read(local_download.join("子目录-γ").join("报告-β.md"))?,
        nested_content
    );
    assert_eq!(
        fs::read(
    ...
    assert!(!local_download.join("过期-本地.txt").exists());

    // Cancel immediately after registration while the job is still queued or
    // entering its first SFTP attempt. The generated payload ensures an
    // independently scheduled attempt cannot complete before the request.
    let cancellation_source = workspace.root.join("取消-重试-源.bin");
    let cancellation_content = generated_fixture_content(16 * 1024 * 1024);
    fs::write(&cancellation_source, &cancellation_content)?;
    let cancellation_remote = format!("{remote_directory}/取消-重试-目标.bin");
    let cancellation_job = registry.start_upload(
        host.clone(),
        UploadRequest {
            local_path: path_text(&cancellation_source)?,
            remote_path: cancellation_remote.clone(),
            conflict_policy: ConflictPolicy::Overwrite,
            recursive: false,
        },
    )?;
    let cancelling = registry.cancel(&cancellation_job.id).await?;
    assert_eq!(cancelling.state, TransferState::Cancelling);
    let cancelled = wait_for_transfer(&registry, &cancellation_job.id).await?;
    assert_eq!(cancelled.state, TransferState::Cancelled, "{cancelled:?}");

    let retried = registry.retry(&cancellation_job.id, host.clone())?;
    assert_eq!(retried.state, TransferState::Queued);
    assert!(retried.revision > cancelled.revision);
    let retried = wait_for_transfer(&registry, &cancellation_job.id).await?;
    assert_completed(&retried)?;

    let cancellation_download = workspace.root.join("取消-重试-下载.bin");
    let retry_download = registry.start_download(
        host.clone(),
        DownloadRequest {
            remote_path: cancellation_remote,
            local_path: path_text(&cancellation_download)?,
            conflict_policy: ConflictPolicy::Overwrite,
            recursive: false,
        },
    )?;
    assert_completed(&wait_for_transfer(&registry, &retry_download.id).await?)?;
    assert_eq!(fs::read(cancellation_download)?, cancellation_content);
...
fn generated_fixture_content(bytes: usize) -> Vec<u8> {
    (0..bytes)
...
fn assert_proxy_command_is_isolated(
    ...
    assert!(
        arguments
            .iter()
            .any(|value| value == &global_known_hosts_option)
    );
    assert!(
        arguments
            .iter()
    ...
    assert!(proxy.contains(if cfg!(windows) {
        "\"-F\" \"none\""
    } else {
        "'-F' 'none'"
    }));
    assert!(proxy.contains(&known_hosts_option));
    assert!(proxy.contains(&global_known_hosts_option));
