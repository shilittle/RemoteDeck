...
impl CommandJobRegistry {
    ...
    pub fn cancel(&self, job_id: &str) -> Result<CommandJob, CommandJobError> {
        let (job, sender) = {
            ...
            revise_command_job(&mut entry.job);
...
fn revise_command_job(job: &mut CommandJob) {
...
async fn execute_job(
    ...
    let mut process = async_command(program);
    process
        .args(args)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
...
fn append_job_output(inner: &RegistryInner, job_id: &str, stream: OutputStream, data: &[u8]) {
    let job = {
        ...
        let changed = {
            ...
            append_bounded(target, truncated, data)
        };
        if !changed {
            return;
        }
...
mod tests {
    ...
    #[test]
    fn legacy_command_snapshot_defaults_revision() {
        ...
        let job: CommandJob = serde_json::from_value(legacy).expect("legacy command job");
        assert_eq!(job.revision, 0);
    }

    #[test]
    fn command_revision_is_monotonic_for_state_and_output_updates() {
        ...
        lock(&registry.inner.entries).insert(
            job.id.clone(),
            RegistryEntry {
                job: job.clone(),
                sequence: 0,
        ...
        assert!(transition_running(&registry.inner, &job.id));
        append_job_output(&registry.inner, &job.id, OutputStream::Stdout, b"one");
        append_job_output(&registry.inner, &job.id, OutputStream::Stdout, b"");
        finish_completed(&registry.inner, &job.id, Some(0));

        let latest = registry.job(&job.id).expect("latest job");
        assert_eq!(latest.state, CommandJobState::Completed);
        assert_eq!(latest.stdout, "one");
        assert_eq!(latest.revision, 4);

        let revisions = std::iter::from_fn(|| events.try_recv().ok())
            .map(|event| event.job.revision)
            .collect::<Vec<_>>();
